...
class AudioManager {
  ...
  // ---------- UI (нажатия кнопок; развешивает uiSounds.ts делегатом) ----------

  /**
   * Нажатие кнопки: сухой короткий клик. Самый частый UI-звук, поэтому тихий и без хвоста —
   * тик-транзиент (шум) + едва слышный блип. Никакого «мультяшного» пиу.
   */
  uiTap(): void {
    this.noise(0.018, 0.05, 2800, 1.4)
    this.tone({ type: 'sine', f0: 900, f1: 1150, dur: 0.045, attack: 0.001, gain: 0.06 })
  }

  /** Назад/закрыть: тот же клик, но нисходящий и ниже — «шаг назад» слышно, не глядя. */
  uiBack(): void {
    this.noise(0.018, 0.04, 1800, 1.4)
    this.tone({ type: 'triangle', f0: 560, f1: 380, dur: 0.075, attack: 0.001, gain: 0.06, lowpass: 2200 })
  }

  /** Тумблер: та же семья, но высота кодирует состояние — вкл выше, выкл ниже. */
  uiToggle(on: boolean): void {
    this.noise(0.016, 0.04, 2400, 1.4)
    this.tone({ type: 'sine', f0: on ? 1046 : 784, dur: 0.06, attack: 0.001, gain: 0.06 })
  }
