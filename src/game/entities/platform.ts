...
/**
 * Платформа (объект сети), типы (конспект 2.5 / 2.9). В скобках — название в UI (Правила):
 *  - vols    — стабильная, вечная опора (белая заливка) → UI «АМС 60 м»
 *  - rrl     — шаткая: через collapseMs после касания разрушается (маджента-контур) → UI «Виртуальная сеть»
 *  - moving  — движется по горизонтали (белая со стрелками) → UI «Передвижная базовая станция»
 *  - fake    — фейк-платформа (препятствие): выглядит как опора, но растворяется без
 *              отскока (проваливаешься), с SafeWall подсвечена → UI «Фейк»
 *
